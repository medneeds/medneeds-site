...
import { API_HOST, PROFILE_PICTURE_PLACEHOLDER } from "@/config/constants.ts";
import { Job, Media, Profile } from "@/config/types.ts";
import { authService } from "@/services/auth/AuthService.ts";
import { Where } from "payload";
import { toBrazilTimezone, toCalendarDate } from "@/utils/dateFormatter.ts";
import dayjs from "dayjs";
import {formatCurrency} from "@/utils/numberFormatter.ts";
...
export function getJobTags(job: Job): JobCardTag[] {
  ...
  if (job.priceInCents) {
    tags.push({
      label: `${formatCurrency(job.priceInCents)} ${job.paymentMethod}`,
      color:
