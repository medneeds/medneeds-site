...
export const tokenSchema = z.object({
    token: z.string().min(1, "Token é obrigatório").max(999),
    loginType: z.enum(['personal', 'institutional']).default('personal'),
    cpf: z.string().optional(),
}).refine((data) => {
    if (data.loginType === 'institutional') {
        return !!data.cpf && data.cpf.replace(/\D/g, '').length === 11;
    }
    return true;
}, { message: "CPF é obrigatório (11 dígitos)", path: ["cpf"] });
