...
export type InstitutionalJobWithPaidAt = InstitutionalJob & { paidAt?: string | null };

interface Props {
  job: InstitutionalJobWithPaidAt | null;
  isOpen: boolean;
  onClose: () => void;
  onMarkPaid: (jobId: string, date: string) => Promise<void>;
  onUnmarkPaid: (jobId: string) => Promise<void>;
}
...
const formSchema = z
  ...
  .refine(
    (data) => !(data.isPaid && !data.paymentDate),
    { message: "A data de pagamento é obrigatória", path: ["paymentDate"] },
  );

type FormValues = z.infer<typeof formSchema>;

export function InstitutionalTransfersDetailsModal({ job, isOpen, onClose, onMarkPaid, onUnmarkPaid }: Props) {
  const [isUpdating, setIsUpdating] = useState(false);

  const form = useForm<FormValues>({
    resolver: zodResolver(formSchema),
    defaultValues: { isPaid: false, paymentDate: new Date() },
  });
  ...
  useEffect(() => {
    if (job && isOpen) {
      ...
      form.setValue("paymentDate", paid ? new Date(job.paidAt!) : new Date());
    }
  }, [job, isOpen, form]);
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      if (values.isPaid && values.paymentDate) {
        ...
        const dateChanged = wasPaid
          ? format(new Date(job.paidAt!), "yyyy-MM-dd") !== dateStr
          : true;
        if (!wasPaid || dateChanged) {
          await onMarkPaid(job.id, isoDate);
        }
      } else if (wasPaid && !values.isPaid) {
        await onUnmarkPaid(job.id);
      }
      onClose();
    } finally {
      setIsUpdating(false);
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="flex items-center justify-between space-y-0">
                      <div>
                        <p className="text-base font-semibold text-foreground mb-1">Status da Transferência</p>
                ...
                {isPaidWatch && (
                  <div className="pt-4 border-t border-border/50 animate-in fade-in slide-in-from-top-2">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <p className="text-sm font-medium text-foreground mb-2">Data de Pagamento</p>
                          <FormControl>
