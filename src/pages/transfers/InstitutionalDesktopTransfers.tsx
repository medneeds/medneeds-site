...
import {
  InstitutionalTransfersDetailsModal,
  type InstitutionalJobWithPaidAt,
} from "./InstitutionalTransfersDetailsModal.tsx";
import { statusColors, statusLabels } from "./utils/constants.ts";

export default function InstitutionalDesktopTransfers() {
  const { institutions } = useInstitutionalContext();

  const [allJobs, setAllJobs] = useState<InstitutionalJobWithPaidAt[]>([]);
  const [loading, setLoading] = useState(false);
  const [statusFilter, setStatusFilter] = useState<string | undefined>();
  const [selectedJob, setSelectedJob] = useState<InstitutionalJobWithPaidAt | null>(null);
  ...
  const fetchJobs = useCallback(async () => {
    ...
    try {
      ...
      setAllJobs(res.docs as InstitutionalJobWithPaidAt[]);
    } catch (e) {
      console.error("[InstitutionalTransfers] fetch error", e);
    } finally {
      setLoading(false);
    }
  ...
  const filteredItems = useMemo(() => {
    ...
    if (statusFilter === "paid") {
      result = result.filter((item) => !!item.paidAt);
  ...
  const summary = useMemo(() => {
    ...
    return {
      paidIncome,
      pendingIncome,
      paidCount: base.filter((j) => j.paidAt).length,
      pendingCount: base.filter((j) => !j.paidAt).length,
    };
  }, [allJobs]);
  ...
  const handleMarkPaid = async (jobId: string, date: string) => {
    await institutionalService.markJobPaid(jobId, date);
    setAllJobs((prev) =>
      prev.map((j) => (j.id === jobId ? { ...j, paidAt: date } : j)),
    );
  };

  const handleUnmarkPaid = async (jobId: string) => {
    await institutionalService.unmarkJobPaid(jobId);
    setAllJobs((prev) =>
      prev.map((j) => (j.id === jobId ? { ...j, paidAt: null } : j)),
    );
  };
        ...
        <div className="bg-card rounded-md p-5 border border-border shadow-card hover:border-primary/20 transition-colors">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground">{summary.paidCount} itens</p>
            </div>
        ...
        <div className="bg-card rounded-md p-5 border border-border shadow-card hover:border-primary/20 transition-colors">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground">{summary.pendingCount} itens</p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-foreground">
                {formatCurrency(summary.paidIncome + summary.pendingIncome, hideValues)}
              </p>
              <p className="text-sm text-muted-foreground">
                {summary.paidCount + summary.pendingCount} itens
              </p>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="flex-1">
              <div className="flex w-fit bg-card rounded-lg p-1 border border-border/50">
                ...
                <Button
                  variant="ghost" size="sm"
                  className={cn("px-4 font-medium", statusFilter === "paid" && "bg-background shadow-sm text-emerald-600")}
      ...
      >
        {loading ? (
          <div className="flex items-center justify-center py-20">
            ...
            <span className="ml-3 text-muted-foreground font-medium">Carregando transferências...</span>
          </div>
