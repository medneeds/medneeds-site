...
interface TransferenciaBulkEditModalProps {
  isOpen: boolean;
  selectedCount: number;
  onClose: () => void;
  onConfirm: (isPaid: boolean, date: string | null) => Promise<void>;
...
export function TransfersBulkEditModal({
  isOpen,
  selectedCount,
  onClose,
  onConfirm,
  isUpdatingBulk = false,
}: TransferenciaBulkEditModalProps) {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    await onConfirm(values.isPaid, newDate);
  };

  return (
    <ResponsiveDialogRoot open={isOpen} onOpenChange={onClose} maxWidth="35vw">
      <ResponsiveDialogHeaderComp title="Editar Transferências" />
      <Form {...form}>
            ...
            <div className="flex flex-col gap-6">
              <div className="text-center mb-2">
                <p className="text-sm font-medium text-muted-foreground">
                  ...
                  transferência{selectedCount > 1 ? "s" : ""}.
                </p>
              </div>
